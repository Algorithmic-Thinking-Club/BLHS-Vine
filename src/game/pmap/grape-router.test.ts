...
/* a real station name off the Maw's own table, so this test breaks if the table
 * is emptied rather than passing vacuously against a name nobody uses */
...
describe('who answers to an anchor', () => {
  ...
  it('ASKS THE ISLAND FIRST, so the vine queues behind a member like anyone else', () => {
    /* the ruling, and the reason it is a ruling: an author on their own map has
     * to be able to take over an anchor the vine already answers to, without
     * asking anybody and without the engine growing a special case */
  ...
  it('does not match a handler that is not a talk handler', () => {
    /* `start` is called by the engine when the island loads and is not an
     * anchor. An island registering only on_start owns no anchors at all. */
...
describe('what the player reads', () => {
  ...
  it('falls back to the anchor s own name for an island', () => {
    /* a member names the handler after the anchor and never writes a label, so
     * the name is the honest last resort rather than something assembled out of
     * the handler key */
...
describe('whether pressing E would do anything', () => {
  /* THIS AND fire() HAVE TO AGREE. The prompt decides whether E is ever offered,
   * so a grape-owned anchor the prompt did not know about is a handler that can
   * never be reached by a key or by a tap. */
  const save = { year: 1 }

  it('is ready when an island claims it, with or without a run', () => {
    const owner = ownerOf('coach', ['talk:coach'])
    expect(isReady(owner, save)).toBe(true)
    /* a grape does not need a save to say a line, and requiring one would mean
     * an island cannot speak to somebody who has not joined a class */
