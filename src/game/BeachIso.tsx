...
// shoreline: sea where (tx+ty) is small (far/back), beach in front. Wavy along the (tx-ty) axis.
const shoreAt = (d: number) => 30 + 5 * Math.sin(d * 0.22) + 2.6 * Math.sin(d * 0.55 + 1.3)
    ...
    const start = async () => {
      ...
      for (let ty = 0; ty < ROWS; ty++) {
        ...
        for (let tx = 0; tx < COLS; tx++) {
          ...
          const isSea = c === 'sea'
          const base = isSea ? (hash(tx, ty) < 0.5 ? tex['water'] : tex['water2']) : tex['sand']
          if (!base) continue
          const sp = new Sprite(base); sp.anchor.set(0.5, 0.25)
          const fx = hash(tx * 3, ty * 7) > 0.5 ? -1 : 1
          const os = isSea ? 1 : 1.06   // oversize land tiles a hair so they overlap and hide the grid seams
          sp.scale.set(fx * os, os)
          sp.position.set(isoX(tx, ty), isoY(tx, ty)); sp.zIndex = (tx + ty) * 16
          // tonal variation so sand/sea never read as one flat tiled sheet: big organic patches
          // (low-freq value noise) + a hair of per-tile shade. Sand base is a calmer warm tan.
          if (isSea) { const dep = Math.min(1, (shoreAt(tx - ty) - (tx + ty)) / 26); sp.tint = mix(0x7fded2, 0x14687a, dep); waterSprites.push({ sp, ph: (tx + ty) * 0.5 }) }
          else if (c === 'wet') sp.tint = mix(0xd8c08a, 0xc9ad78, hash(tx, ty))
          else {
            const big = vnoise(tx / 9 + 3, ty / 9 + 5), med = vnoise(tx / 3.5 + 1, ty / 3.5 + 9), fine = hash(tx * 1.3, ty * 2.1)
            const drift = big * 0.7 + med * 0.3
            const warm = mix(0xcdb87e, 0xedd9a6, drift)          // wide warm/pale dune patches
            sp.tint = shadeHex(warm, 0.95 + fine * 0.08)
          }
          world.addChild(sp)
      ...
      // ---- ground decals: scattered pebbles / shells / ripple flecks so the sand reads as a
      // textured surface, not a tiled grid (the campus edge-scatter trick) ----
      const pebbleTex = makeFleck(0x9c8e72, 0x6f6552), shellTex = makeFleck(0xf2e3c8, 0xd9a9a0)
      for (let ty = 3; ty < ROWS - 3; ty++) for (let tx = 3; tx < COLS - 3; tx++) {
        if (cellAt(tx, ty) !== 'sand') continue
        const r = hash(tx * 9.1, ty * 4.3)
        if (r > 0.86) {
          const t = r > 0.95 ? shellTex : pebbleTex
          const jx = (hash(tx, ty) - 0.5) * 28, jy = (hash(ty, tx) - 0.5) * 12
          const d = new Sprite(t); d.anchor.set(0.5, 0.6); d.scale.set(0.5 + hash(tx * 2, ty) * 0.7)
          d.position.set(isoX(tx, ty) + jx, isoY(tx, ty) + jy); d.zIndex = (tx + ty) * 16 + 4
          world.addChild(d)
        }
      }

      // ---- props: upright iso billboards, depth-sorted, grounded shadow, collision ----
      ...
      // ---- Thor ----
      const thor = new Sprite(idle['south'] ?? tex['palmB']); thor.anchor.set(0.5, 0.9); thor.scale.set(0.62)
      thor.zIndex = 0; world.addChild(thor)
      const pos = { tx: 24, ty: 20 }; let facing = 'south', at = 0
      ...
      instance.ticker.add((tk) => {
        ...
        for (const w of waterSprites) w.sp.alpha = 0.9 + 0.1 * Math.sin(tnow * 1.6 + w.ph)
        resizeFx(vw, vh)
      })
