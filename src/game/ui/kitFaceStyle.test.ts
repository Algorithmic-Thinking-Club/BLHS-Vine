...
/* the sheet as published: 512x256, eight cut faces, no slice and no css because a
 * grid of small marks is not a stretchable surface */
...
/* the band as published: a nine-slice whose two marked rectangles are exactly the
 * paper between the frames, 47/55 across and 28/29 down */
const BAND: KitPiece = {
...
describe('cutting one mark off a sheet', () => {
  ...
  it('slides it by the LEFTOVER travel, not by the sheet width', () => {
    /* the percentage trap: background-position aligns the same percentage point
     * of the image with that point of the box, so the denominator is w - faceW.
     * Using the sheet width puts every face a little to the left of itself. */
  ...
  it('refuses a face nobody drew, so the caller keeps its glyph', () => {
    // the live sheet has no book and no anchor, which is why two HUD buttons and
    // all six Handbook badges still show an emoji
  ...
  it('refuses when the platform kit is not worn, because the token would be empty', () => {
    // `applyKit` only runs behind ?kit=1, so without it var(--kit-art-icon_set)
    // resolves to nothing and the button would be a blank square
...
describe('dressing the arrival band off its own marked rectangles', () => {
  ...
  it('agrees with the piece own nine-slice, which is the check that it was read right', () => {
    // the paper inside a nine-slice IS the marked area; if these ever disagree,
    // one of the two was measured and the other was guessed
    const d = bandDress([BAND], true)
  ...
  it('still measures the layout when the art is not worn', () => {
    /* ?kit=1 is a decision about whether the platform PAINTS. Where the name of a
     * place sits inside the band is not that decision, so the fallback plaque gets
     * the authored insets too and the flag changes the frame, not the layout. */
